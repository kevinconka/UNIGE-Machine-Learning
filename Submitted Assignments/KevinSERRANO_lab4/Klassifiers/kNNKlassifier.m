classdef kNNKlassifier < handle
    ...
    properties
        ...
        distfcn = 'Euclidean'       % by default ['Manhattan']
        
        % neighbor weight contribution
        weightfcn = 'equal'         % no weighting (default) ['invdist', 'rank']
    ...
    methods
        ...
        function [y_pred, k_nn] = predict(obj, X)
            ...
            for i = 1:m
                ...
                switch(obj.distfcn)
                    case 'Manhattan'
                        dist = sum(X_i - obj.raw_X, 2); 
                    otherwise % Euclidean squared
                        dist = sum((X_i - obj.raw_X).^2, 2); 
                end
                [D, I] = sort(dist);    % sort from min dist to max dist
                
                % get the respective k-nn and store it in table form (class, distance, rank)
